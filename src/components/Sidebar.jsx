...
export function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-30 w-64 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-800 text-white transform transition-transform duration-300 ease-in-out lg:translate-x-0 shadow-2xl ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between h-20 px-6 border-b border-gray-700/50 bg-gray-900/50 backdrop-blur-sm">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">AI CRM</span>
            </div>
            <button
              onClick={() => setIsOpen(false)}
          ...
          <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            {navigation.map((item, index) => {
              ...
              return (
                ...
                >
                  <svg className={`w-5 h-5 mr-3 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {item.icon}
                  </svg>
            ...
            <div className="flex items-center space-x-3 mb-4 p-3 rounded-xl bg-gray-800/50 hover:bg-gray-800 transition-colors">
              <div className="relative">
                <img
                  ...
                  alt={user?.name || 'User'}
                  className="w-12 h-12 rounded-xl ring-2 ring-blue-500/30"
                />
